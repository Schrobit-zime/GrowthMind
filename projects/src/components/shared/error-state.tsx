...
export function ErrorState({ title = "加载失败", message = "请检查网络连接后重试", onRetry, className }: ErrorStateProps) {
  return (
    <div className={`flex flex-col items-center justify-center py-16 ${className || ""}`}>
      ...
      {onRetry && <Button onClick={onRetry} variant="outline" className="backdrop-blur-md bg-white/5 border-white/10 text-white gap-2"><RefreshCw className="w-4 h-4"/>重试</Button>}
    </div>
  );
}
